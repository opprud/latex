...
Assesing inititatives

\subsection{grouping into adopter segments}
In table ~\ref{table:asses_init}, third column the different initiatives are assessed, and characterized among each other. Characterization is done in relation to the model attempting to identify one or more segments the campaign has successfully targeted. 

\subsection{stimulating adoption}
The fourth column in table ~\ref{table:asses_init} attempt to rate the campaigns aim, at each of Kaplans factors, stimulating adoption.
...
\subsection{Arguing the table...}
This article has primarily focused at four recent initiatives, implemented in Denmark, where two of the initiatives is actually targeted at sectors outside the area of interest, the households. The reason for this, is that the campaign \textit{Chasing the hidden energy} tries to change the mindset, at peoples job, but aiming at this mindset-change to be deployed in their private households as well. The tool \textit{Se-Elforbrug} is interesting because it asses some of the possibilities leveraged, to users, when deploying remote and collective sensing, such as data-sharing, collective measuring, enforcing companies and institutions to comped against each other, in lowering consumption.\newline
...
\textbf{Current Phase}: describes the current phase, along with the authors perception on, where in its life-cycle the campaign currently resides. Based on its \textit{current Phase}, being \textit{Growing, Stable} or \textit{Declining}, a score is assigned, being the value \textbf{1} for\textit{Declining}, \textbf{3} for \textit{Stable} and 10 for \textit{Declining}. 

\textbf{Potential}: attempts to look into the crystal ball, calculating a score the aims at predicting the future for this campaign. The score is calculated based on a multiplication and weighing of the three previous factors.  
...
Table ~\ref{table:score} summarizes the perception and tries to present predictions for these campaigns. The results are mapped in a coordinate system, helping to visualize the perception of the campaigns.
