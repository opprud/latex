...
This article assesses some of the current and past campaigns, with an aim of indicating what instruments have been used to reach energy consumers, and present saving potentials. Thus the aim is to get an indication of what effective instruments can be utilized in future IT-aided tools, for enforcing greener habits.

The articles outcome indicates that campaigns that stimulates greener habits, and targeted initiatives have an impact. 
Public campaigns are in general perceived as trustworthy, though they need time to be perceived by the end users.
...
An example of this is presented, assessing energy-using consumer durables \cite{newell}. The example maps development of air-conditioning systems in terms of energy efficiency, and relates it to the previous decades increase in energy prices, and argues that the increase in energy-cost has pushed energy-saving technology forward.\newline
...
Initially Rogers models \cite{rogers_model} operates with a model describing market penetration described by an S curve, also known as the logistic function, figure ~\ref{fig:adoption}
...
Adopting an innovation requires a number of decisions. The decisions happens sequentially over time, and are by Rogers \cite{rogers_model} described as a five stage process, figure ~\ref{fig:iv_process}.
\begin{figure}
...
Kaplan strongly argues and proves that Rogers five factors, introduced in the previous section, in not operational enough as a model, and is lacking a few factors. Kaplan presents and proofs a model\cite{passive_to_active}, inspired from Rogers, but it has following key differences, in the first two of the five steps, before the third step \textit{Decision} \cite{passive_to_active}: \newline
...
\textit{Development of new variables, figure ~\ref{fig:kaplan_model}:} \newline
...
The campaign was established in 2010, and is led by \textit{Center For Energy-savings}, an organization founded by the Ministry of Climate- and Energy. Funding of the campaign and the organization is a tax of 0,006 DKK/KWh, paid by customers in the households and public sector. \textit{Center For Energy-savings} succeeds the former organization \textit{Elsparefonden}, who had a dedicated focus on saving electricity \cite{annual_ens_10}. 
...
Also training targeted at craftsmen, such as carpenters and electricians, are conducted. This to leverage the ability of competent guidance, and presenting energy savings, well proved mature solutions, when renovating older buildings, or crafting new buildings \cite{annual_ens_11}.  

Besides from the above, \textit{Go energi} provides evaluations, tools, and training for  public sectors, institutions etc. beyond the scope of this article. \newline
...
\textbf{Popularity}: is based on the the variables in \textit{Kaplans} modified model,  figure ~\ref{fig:kaplan_model}, in table ~\ref{table:asses_init}, the factors are assigned values increasing, as the factor gets closer to stimulating interest (i.e right-most variables).

Thus \textit{Motivation} and \textit{Context} has the value \textbf{10}, \textit{Experience} and \textit{Knowledge} the value \textbf{20} and \textit{Familarity} the value \textbf{40}. 
Each variable will be assigned a weight being \textbf{1},\textbf{2} or \textbf{3}. A variable is multiplied with its rating, regarding how it its percieved to stimulate the adoption factor of interest, being \textit{low} weighing \textbf{1}, \textit{medium} weighing \textbf{2}, and \textit{high} being \textbf{3}.
Thus popularity will be made up of a sum of all five factors, each multiplied with their rating, for each of the four initiatives.\newline

\textbf{Maturity}: indicates the reached target-audience, in terms of adopter-groups, described by \textit{Rogers}, by means of the logistic curve, or S-curve. 
To get a rating, the segment or segments reached, has the center-weight of the rightmost group, in the logistic curve figure ~\ref{fig:iv_process}, hereby reaching the score,  having the highest rate, if more groups are targeted. E.g. \textit{late majority gets the score 75}. This will provide a good measure of the targeted audience.\newline

\textbf{Current Phase}: describes the current phase, along with the authors perception on, where in its life-cycle the campaign currently resides. Based on its Current Phase, being \textit{Growing, Stable} or \textit{Declining}, a score is assigned, being the value \textbf{1} for \textit{Declining}, \textbf{3} for \textit{Stable} and \textbf{10} for \textit{Increasing}. The exponential increase in value, \textbf{1}, \textbf{3} and \textbf{10} accordingly, represents the fact that market-penetration of new technologies follows an exponential pattern, again known as the \textit{S-curve}\cite{rogers_model}, and hereby the value is derived from the slope of the curve.  \newline
...
This model, as per today, just multiplies, and scales the numbers presented above directly, and maps the \textbf{Potential} as the ultimate result. Whether this is adequate, and an acknowledgment that the above assumptions might be a bit simplified, is neglected for now, since this is a first attempt to establish a framework for assessment of this type of initiatives.
 \newline 
...
Initiative & Charachteristics & adopter group, according to Rogers & Stimulation of adoption variables according to Kaplan\\ [0.5ex] % inserts table 
%heading
\hline                  % inserts single horizontal line
...
If success is achieved in enforcing new habits, the marked for IT-aided tools, like smartphone-apps, that helps the user, will definitively be mature of adopting new innovation and initiatives presented.  
